...
if(isset($_POST["logar"])) { // Verifico se o botão de login foi acionado
	...
	if(!empty($_POST["login"]) && !empty($_POST["senha"])) { // Verifico se os campos foram preenchidos
		...
		$valida_user = mysqli_query($conecta, "SELECT * FROM $tabela WHERE nome='$login' AND senha='$senha'") or die(mysqli_error()); // Faço a consulta ao SQL para buscar o usuário com os dados informados pelo form
