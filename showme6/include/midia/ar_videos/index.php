...
if($rsecveradm['adm'] == 1){
	$csql = mysql_query("SELECT * FROM videos WHERE id_ar = '$i2';");
	...
	if($rsecveradm['adm'] == 1){
		if(isset($_POST["alterar"])) {
			if(!empty($_POST["nome"]) && !empty($_POST["descricao"])) {
				...
				$altera = mysql_query("update videos set nome = '$nome', descricao = '$descricao' where id = '$i2';");
				if ($altera){ 
...
if($i1 == 3){
	...
	$rsecveradm = mysql_fetch_array($secveradm);
	if($rsecveradm['adm'] == 1){
		$altera = mysql_query("delete from videos where id = '$i2';");
...
$insert = mysql_query("insert into videos (id_ar, link, nome, descricao) values ('$i2', '$nome_final', '$nome', '$descricao');");
if($insert){
