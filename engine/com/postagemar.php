...
$res1 = mysqli_query($conecta, "SELECT * FROM `post_ar` where id_ar in ($contatos) ORDER BY id DESC LIMIT $numincial , $exibicoesporpag");
while($escrever1=mysqli_fetch_array($res1)){
	$e = $e + 1;
	$res = mysqli_query($conecta, "select * from artista where id = '$escrever1[id_ar]'");
	$escrever=mysqli_fetch_array($res);
	$datatempo = explode(" ", $escrever1['data']);
	$dat = explode("-", $datatempo[0]);
	$like = mysqli_query($conecta, "select * from gostar_ar where id_post = '$escrever1[id]' and gostei = '1'");
	$rlike = mysqli_num_rows($like);
	$nlike = mysqli_query($conecta, "select * from gostar_ar where id_post = '$escrever1[id]' and gostei = '0'");
	$rnlike = mysqli_num_rows($nlike);
	$iddouser = $escrever1['id_ar'];
	$estmus = mysqli_query($conecta, "select * from est_musical where id = '$escrever[est_musical]';");
	...
	$res3 = mysqli_query($conecta, "SELECT * FROM `repost_ar` where id_post = '$escrever1[id]' ORDER BY id asc LIMIT 0 , 30");
...
$contagemdepags = mysqli_query($conecta, "SELECT count(*) FROM `post_ar` where id_ar in ($contatos);");
