...
function cabeca(){
	...
	if(isset($_POST["post"])) { 
		if(!empty($_POST["msg"]) && !empty($_POST["id_tipo"]) && !empty($_POST["id_subtipo"])) {
			...
			echo $query;
			$insert = mysqli_query($query) or die(mysqli_error());
			if($insert) {
	...
	if($p1 == 1){
		if($row['idart'] == null){
			$csql = mysqli_query($conecta, "select * from gt where id_us = '$id' and id_post = '$p2';");
			if(mysqli_num_rows($csql) > 0){
				$insert = mysqli_query($conecta, "update gt set gostei = '$p3', data = '$date' where id_post = '$p2' and id_us = '$id'");
				if($insert){ echo "<script>window.location='" . $pagina . "';</script>"; }else{ echo "<script>alert('error');window.location='" . $pagina . "';</script>"; }
			} else {
				$insert = mysqli_query($conecta, "insert into gt (id_post, id_us, gostei, data) values('$p2', '$id', '$p3', '$date')");
				if($insert){ echo "<script>window.location='" . $pagina . "';</script>"; }else{ echo "<script>alert('error');window.location='" . $pagina . "';</script>"; }
			}
		}else{
			$csql = mysqli_query($conecta, "select * from gt where id_ar = '$row[idart]' and id_post = '$p2';");
			if(mysqli_num_rows($csql) > 0){
				$insert = mysqli_query($conecta, "update gt set gostei = '$p3', data = '$date' where id_post = '$p2' and id_ar = '$row[idart]'");
				if($insert){ echo "<script>window.location='" . $pagina . "';</script>"; }else{ echo "<script>alert('error');window.location='" . $pagina . "';</script>"; }
			} else {
				$insert = mysqli_query($conecta, "insert into gt (id_post, id_ar, gostei, data) values('$p2', '$row[idart]', '$p3', '$date')");
				if($insert){ echo "<script>window.location='" . $pagina . "';</script>"; }else{ echo "<script>alert('error');window.location='" . $pagina . "';</script>"; }
			}
		}
	}
	...
	if($p1 == 2){
		$alt_csql = mysqli_query($conecta, "SELECT * FROM gt WHERE id = '$p2';");
		$alt_rsql = mysqli_fetch_array($alt_csql);
		if($alt_rsql['id_us'] != null){
			if($alt_rsql['id_us'] == $row['id']){
				$altera = mysqli_query($conecta, "delete from gt where id = '$p2';");
				if($altera){ $msgerror = "Gostar apagado com sucesso"; } else { $msgerror = "Houve um problema"; }
			}
		}else{
			$secveradm = mysqli_query($conecta, "select * from membros where id_us = '$id' and id_ar = '$alt_rsql[id_ar]'");
	...
	if($p1 == 3){
		$apg_csql = mysqli_query($conecta, "SELECT * FROM com WHERE id = '$p2';");
		$apg_rsql = mysqli_fetch_array($apg_csql);
		if($apg_rsql['id_post'] == null){
			if($apg_rsql['id_us'] != null){
				if($apg_rsql['id_us'] == $row['id']){
					$apaga = mysqli_query($conecta, "delete from com where id = '$p2';");
					$apaga = mysqli_query($conecta, "delete from com where id_post = '$p2';");
					if($apaga){ $msgerror = "Postagem apagada com sucesso"; } else { $msgerror = "Houve um problema"; }
				}
			}else{
				$secveradm = mysqli_query($conecta, "select * from membros where id_us = '$id' and id_ar = '$apg_rsql[id_ar]'");
				$rsecveradm = mysqli_fetch_array($secveradm);
				if($rsecveradm['adm'] == 1){
					$apaga = mysqli_query($conecta, "delete from com where id = '$p2';");
					$apaga = mysqli_query($conecta, "delete from com where id_post = '$p2';");
	...
	if($p1 == 4){
		$alt_csql = mysqli_query($conecta, "SELECT * FROM com WHERE id = '$p2';");
		$alt_rsql = mysqli_fetch_array($alt_csql);
		if($alt_rsql['id_us'] != null){
			if($alt_rsql['id_us'] == $row['id']){
				if(isset($_POST["alterapost"])) {
					if(!empty($_POST["msg"])) {
						$msg = $class->antisql($_POST["msg"]);
						$altera = mysqli_query($conecta, "update com set msg = '$msg' WHERE id='$p2';");
...
function post_us($func_id){
	...
	echo $com_rsql['id_us'];
	$csql_user = mysqli_query($conecta, "select * from user where id = '$com_rsql[id_us]'");
	$rsql_user = mysqli_fetch_array($csql_user);
	$datatempo = explode(" ", $com_rsql['data']);
	$dat = explode("-", $datatempo[0]);
	$like = mysqli_query($conecta, "select * from gt where id_post = '$com_rsql[id]' and gostei = '1'");
	$rlike = mysqli_num_rows($like);
	$nlike = mysqli_query($conecta, "select * from gt where id_post = '$com_rsql[id]' and gostei = '0'");
	...
	$res3 = mysqli_query($conecta, "SELECT * FROM `com` where id_tipo = '$com_idtipo' and id_subtipo = '$com_idsubtipo' and id_post = '$com_rsql[id]' ORDER BY id asc LIMIT 0 , 30");
...
function post_ar(){
	$e = $e + 1;
	$csql_artista = mysqli_query($conecta, "select * from artista where id = '$com_rsql[id_ar]'");
	$rsql_artista = mysqli_fetch_array($csql_artista);
	$datatempo = explode(" ", $com_rsql['data']);
	$dat = explode("-", $datatempo[0]);
	$like = mysqli_query($conecta, "select * from gt where id_post = '$com_rsql[id]' and gostei = '1'");
	$rlike = mysqli_num_rows($like);
...
echo"
	...
	while($rrlike = mysqli_fetch_array($like)){
		...
		$csqlrlk = mysqli_query($conecta, "select * from artista where id = '$rrlike[id_ar]'");
		$rsqlrlk = mysqli_fetch_array($csqlrlk);
		$datatempo5 = explode(" ", $rrlike['data']);
		$dat5 = explode("-", $datatempo5[0]);
		$secveradm = mysqli_query($conecta, "select * from membros where id_us = '$id' and id_ar = '$rrlike[id_ar]'");
		$rsecveradm = mysqli_fetch_array($secveradm);
	...
	while($rrnlike = mysqli_fetch_array($nlike)){
		...
		$csqlrnlk = mysqli_query($conecta, "select * from artista where id = '$rrnlike[id_ar]'");
		$rsqlrnlk = mysqli_fetch_array($csqlrnlk);
		$datatempo6 = explode(" ", $rrnlike['data']);
		$dat6 = explode("-", $datatempo6[0]);
		$secveradm = mysqli_query($conecta, "select * from membros where id_us = '$id' and id_ar = '$rrnlike[id_ar]'");
		$rsecveradm = mysqli_fetch_array($secveradm);
...
<div id=\"boxes\">
	...
	<span class=textoinfo>" . $dat[2] . "/" . $dat[1] . "/" . $dat[0] . " " . $datatempo[1] . " ";
	$secveradm = mysqli_query($conecta, "select * from membros where id_us = '$id' and id_ar = '$com_rsql[id_ar]'");
	$rsecveradm = mysqli_fetch_array($secveradm);
	if($rsecveradm['adm'] == 1){
		echo "<br>
...
$csqlus = mysqli_query($conecta, "select * from user where id = '$escrever3[id_us]';");
		...
		$like2 = mysqli_query($conecta, "select * from $tabelagostar where id_post = '$escrever3[id]' and gostei = '1'");
		$rlike2 = mysqli_num_rows($like2);
		$nlike2 = mysqli_query($conecta, "select * from $tabelagostar where id_post = '$escrever3[id]' and gostei = '0'");
...
$csqlar = mysqli_query($conecta, "select * from artista where id = '$escrever3[id_ar]';");
 		$rsqlar = mysqli_fetch_array($csqlar);
 		$datatempo2 = explode(" ", $escrever3['data']);
		$dat2 = explode("-", $datatempo2[0]);
		$like2 = mysqli_query($conecta, "select * from $tabelagostar where id_post = '$escrever3[id]' and gostei = '1'");
		$rlike2 = mysqli_num_rows($like2);
...
function remenu_us(){
	echo "
		...
		while($rrlike2 = mysqli_fetch_array($like2)){
			...
			$csqlrlk2 = mysqli_query($conecta, "select * from artista where id = '$rrlike2[id_ar]'");
			$rsqlrlk2 = mysqli_fetch_array($csqlrlk2);
			$datatempo3 = explode(" ", $rrlike2['data']);
			$dat3 = explode("-", $datatempo3[0]);
			$secveradm = mysqli_query($conecta, "select * from membros where id_us = '$id' and id_ar = '$rrlike2[id_ar]'");
			$rsecveradm = mysqli_fetch_array($secveradm);
		...
		while($rrnlike2 = mysqli_fetch_array($nlike2)){
			...
			$csqlrnlk2 = mysqli_query($conecta, "select * from artista where id = '$rrnlike2[id_ar]'");
			$rsqlrnlk2 = mysqli_fetch_array($csqlrnlk2);
			$datatempo4 = explode(" ", $rrnlike2['data']);
			$dat4 = explode("-", $datatempo4[0]);
			$secveradm = mysqli_query($conecta, "select * from membros where id_us = '$id' and id_ar = '$rrnlike2[id_ar]'");
			$rsecveradm = mysqli_fetch_array($secveradm);
...
function remenu_ar(){	
		...
		<a href=\"#rmenu" . $i . "\" name=\"fechamenu\" class=\"classe4\"><img src=\"engine/imgs/cancela.png\"></a>
				...
				<span class=textoinfo>" . $dat2[2] . "/" . $dat2[1] . "/" . $dat2[0] . " " . $datatempo2[1] . " ";
		$secveradm = mysqli_query($conecta, "select * from membros where id_us = '$id' and id_ar = '$escrever3[id_ar]'");
		$rsecveradm = mysqli_fetch_array($secveradm);
		if($rsecveradm['adm'] == 1){
			echo "<br>
			<input id=\"cordoinput\" type=\"button\" value=\"Alterar\" onClick=\"ocmenualt('" . $i . "')\">
...
$com_csql = mysqli_query($conecta, "select * from com where id_tipo = '$com_idtipo' and id_subtipo = '$com_idsubtipo' and (id_post IS NULL) order by id desc limit $numincial, $exibicoesporpag;");
while($com_rsql=mysqli_fetch_array($com_csql)){
...
$contagemdepags = mysqli_query($conecta, "SELECT count(*) FROM `com` where id_tipo = '$com_idtipo' and id_subtipo = '$com_idsubtipo' and (id_post IS NULL);");
