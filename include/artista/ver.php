...
$csql4 = mysqli_query($conecta, "SELECT * FROM membros WHERE id_us = '$id' and id_ar = '$i2'");
$rsql4 = mysqli_fetch_array($csql4);
	if ($rsql['musicas'] == 1){ $mus = "Proprias"; }
	if ($rsql['musicas'] == 2){ $mus = "Covers"; }
	if ($rsql['musicas'] == 3){ $mus = "Mistas"; }
$seguir = mysqli_query($conecta, "SELECT * FROM seguir WHERE deid='$id' and artid='$i2'");
...
<span class=\"titulo\">Musicas:</span><br>";
	$musicascsql = mysqli_query($conecta, "SELECT * FROM musicas WHERE id_ar = '$i2' limit 0 , 2;");
...
$disccsql = mysqli_query($conecta, "SELECT * FROM disc WHERE id_ar = '$i2' limit 0,3;");
...
if ($i1 == 2){
$add = mysqli_query($conecta, "INSERT INTO seguir(deid, artid) VALUES('$id', '$i2')") or die(mysqli_error()); 
if($add) {
...
if ($i1 == 3){
$add = mysqli_query($conecta, "delete from seguir where deid = '$row[id]' and artid = '$i2'") or die(mysqli_error()); 
if($add) {
echo "<script>window.location='" . $pagina . "';</script>";
}
else {
