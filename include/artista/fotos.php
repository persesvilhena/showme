...
$csql2 = mysqli_query($conecta, "select * from membros where id_ar = '$rsql[id]' and id_us = '$id'");
$rsql2 = mysqli_fetch_array($csql2);
$csql3 = mysqli_query($conecta, "select * from user where id = '$rsql2[id_us]'");
...
if (isset($_POST['alterar'])) {
	...
	$sec = mysqli_query($conecta, "select * from album_ar where id = '$idimg'");
	$rsec = mysqli_fetch_array($sec);
	$sec2 = mysqli_query($conecta, "select * from membros where id_ar = '$rsec[us]' and id_us = '$id';");
	$rsec2 = mysqli_fetch_array($sec2);
	if ($rsec2['adm'] == 1){
		$alte = mysqli_query($conecta, "update album_ar set descricao = '$descricao' where id = '$idimg'");
		if($alte){ 
...
if (isset($_POST['cadastrar'])) {
	...
	if (!empty($foto["name"])) {
		...
		if (count($error) == 0) {
			...
			$sql = mysqli_query($conecta, "INSERT INTO album_ar VALUES (null, '$descricao', '$us', '$nome_imagem')");
...
if($i1 == 2){
	$qfoto = mysqli_query($conecta, "select * from album_ar where id = '$i3';");
...
if($i1 == 4){
	$qfoto = mysqli_query($conecta, "select * from album_ar where id = '$i2';");
	$rfoto = mysqli_fetch_array($qfoto);
	$sec = mysqli_query($conecta, "select * from membros where id_ar = '$rfoto[us]' and id_us = '$id';");
	$rsec = mysqli_fetch_array($sec);
	if ($rsec['adm'] == 1){

		unlink("albumar/".$rfoto['nome']."");

		$exc = mysqli_query($conecta, "delete from album_ar where id = '$i2'");
		if($exc){ 
