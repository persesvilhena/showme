...
if($rsecveradm['adm'] == 1){
	...
	if(isset($_POST["cadastrar"])) {
		if(!empty($_POST["descricao"]) && !empty($_POST["data"]) && !empty($_POST["hora"]) && !empty($_POST["local"]) && !empty($_POST["link"])) { 
			...
			$insert = mysqli_query($conecta, "INSERT INTO agenda(id_ar, descricao, data, hora, local, link, rev) VALUES('$i2', '$descricao', '$data', '$hora', '$local', '$link', '0')") or die(mysqli_error());
			if($insert) {
	...
	if(isset($_POST["alterar"])) {
		if(!empty($_POST["descricao"]) && !empty($_POST["data"]) && !empty($_POST["hora"]) && !empty($_POST["local"]) && !empty($_POST["link"])) { 
			...
			$insert = mysqli_query($conecta, "update agenda set descricao = '$descricao', data = '$data', hora = '$hora', local = '$local', link = '$link', rev = '0' where id_ar = '$i2';") or die(mysqli_error());
			if($insert) {
	...
	if($i1 == 1){
	$res = mysqli_query($conecta, "SELECT * FROM agenda where id_ar = '$i2'");
	...
	if($i1 == 3){
	$deleta = mysqli_query($conecta, "delete from agenda where id = '$i3';");
	if($deleta){
