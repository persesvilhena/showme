...
if($i1 == null){$i1 = 1;}
if ($i1 == 1){
$secveradm = mysqli_query($conecta, "select * from membros where id_us = '$id' and id_ar = (select id_ar from disc where id = '$i2')");
$rsecveradm = mysqli_fetch_array($secveradm);
if($rsecveradm['adm'] == 1){
	$csql = mysqli_query($conecta, "SELECT * FROM disc_musicas WHERE id_album = '$i2';");
...
if($i1 == 2){
	...
	$secveradm = mysqli_query($conecta, "select * from membros where id_us = '$id' and id_ar = (select id_ar from disc where id = '$rsql[id_album]')");
	$rsecveradm = mysqli_fetch_array($secveradm);
	if($rsecveradm['adm'] == 1){
		if(isset($_POST["alterar"])) {
			if (empty($_FILES['arquivo']['name'])){
				if(!empty($_POST["nome"])) {
					...
					$altera = mysqli_query($conecta, "update disc_musicas set nome = '$nome', numero = '$numero', composicao = '$composicao', letras = '$letras', duracao = '$duracao', descricao = '$descricao' where id = '$i2';");
					if($altera){ 
					...
					if(!empty($_POST["nome"])) {
						...
						// O arquivo passou em todas as verificações, hora de tentar movê-lo para a pasta
							...
							if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'] . $nome_final)) {
								$insert = mysqli_query($conecta, "update disc_musicas set nome = '$nome', numero = '$numero', composicao = '$composicao', letras = '$letras', duracao = '$duracao', descricao = '$descricao', musica = '$nome_final' where id = '$i2';");
								if($insert){
...
if($i1 == 3){
	...
	$rsecveradm = mysqli_fetch_array($secveradm);
	if($rsecveradm['adm'] == 1){
		$altera = mysqli_query($conecta, "delete from disc_musicas where id = '$i2';");
		if ($rsql['musica'] != null){
		unlink("uploads/musicas/".$rsql['musica']."");
		}
		if ($altera){ 
...
$duracao = $class->antisql($_POST["duracao"]);
$descricao = $class->antisql($_POST["descricao"]);
$insert = mysqli_query($conecta, "insert into disc_musicas (id_album, nome, numero, composicao, letras, duracao, descricao) values ('$i2', '$nome', '$numero', '$composicao', '$letras', '$duracao', '$descricao');");
...
if (move_uploaded_file($_FILES['arquivo']['tmp_name'], $_UP['pasta'] . $nome_final)) {
$insert = mysqli_query($conecta, "insert into disc_musicas (id_album, nome, numero, composicao, letras, duracao, descricao, musica) values ('$i2', '$nome', '$numero', '$composicao', '$letras', '$duracao', '$descricao', '$nome_final');");
if($insert){
// Upload efetuado com sucesso, exibe uma mensagem e um link para o arquivo
...
if($i1 == 6){
	$csql = mysqli_query($conecta, "select * from pt_musica where id_us = '$id' and id_mus = '$i2';");
	if(mysqli_num_rows($csql) > 0){
		$insert = mysqli_query($conecta, "update pt_musica set nota = '$i3' where id_mus = '$i2' and id_us = '$id'");
		...
		$insert = mysqli_query($conecta, "insert into pt_musica (id_mus, id_us, nota) values('$i2', '$id', '$i3')");
		if($insert){ echo "<script>window.location='index.php?discmusicas&i1=5&i2=" . $i2 . "';</script>"; }else{ echo "<script>alert('error');window.location='index.php?discmusicas&i1=5&i2=" . $i2 . "';</script>"; }
	}
