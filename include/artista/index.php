...
if($i1 == null){$i1 = 1;}
if($i1 == 1){
$res = mysqli_query($conecta, "SELECT * FROM membros where id_us = '$id'");
 echo "<div class=\"tabela1\"><table><tr><td>Bandas</td></tr>";
 while($escrever=mysqli_fetch_array($res)){
 	$csql = mysqli_query($conecta, "SELECT * FROM artista WHERE id = '$escrever[id_ar]'");
...
$res2 = mysqli_query($conecta, "SELECT * FROM convite_membro where id_us = '$id'");
 ...
 while($escrever2=mysqli_fetch_array($res2)){
 	$csql2 = mysqli_query($conecta, "SELECT * FROM artista WHERE id = '$escrever2[id_ar]'");
...
if(!empty($_POST["nome"]) && !empty($_POST["cidade"]) && !empty($_POST["estado"])) { 
		...
		$repeat_user = mysqli_query($conecta, "SELECT * FROM artista WHERE nome = '$nome'") or die($mensagem_erro = "Houve um erro:<br />".mysqli_error());
		...
		if(mysqli_num_rows($repeat_user) > 0) { 
			...
			$insert = mysqli_query($conecta, "INSERT INTO artista(nome, descricao, est_musical, musicas, cidade, estado, foto, rev) VALUES('$nome', '$descricao', '$est_musical', '$musicas', '$cidade', '$estado', 'new/new.png', '0')") or die(mysqli_error());
			$msql = mysqli_query($conecta, "select * from artista where nome = '$nome';");
			$rmsql = mysqli_fetch_array($msql);
			$insert = mysqli_query($conecta, "INSERT INTO membros(id_us, id_ar, adm, rev) VALUES('$id', '$rmsql[id]', '1', '0')");
			if($insert) { 
...
<?php
$sql = mysqli_query($conecta, "select * from est_musical order by nome asc");
...
if($i1 == 3){
$csql = mysqli_query($conecta, "SELECT * FROM convite_membro WHERE id = '$i2'");
$rsql = mysqli_fetch_array($csql);
$csql2 = mysqli_query($conecta, "SELECT * FROM artista WHERE id = '$rsql[id_ar]'");
$rsql2 = mysqli_fetch_array($csql2);
echo "
...
$csql = mysqli_query($conecta, "SELECT * FROM convite_membro WHERE id = '$i2'");
$rsql = mysqli_fetch_array($csql);
$rs = mysqli_query($conecta, "SELECT * FROM convite_membro WHERE id = '$i2' and id_us = '$id';") or die($mensagem_erro = "Houve um erro:<br />".mysqli_error()); // Faço a consulta ao SQL para verificar se não há usuários com o mesmo login name
if(mysqli_num_rows($rs) > 0) {
$insert = mysqli_query($conecta, "insert into membros (id_us, id_ar, adm, rev) values('$id', '$rsql[id_ar]', '0', '0');");
$insert = mysqli_query($conecta, "delete from convite_membro where id = '$i2';");
