...
$artista = mysqli_query($conecta, "SELECT * FROM artista WHERE id='$i2'");
$rartista = mysqli_fetch_array($artista);
$adm = mysqli_query($conecta, "SELECT * FROM membros WHERE id_us='$id' and id_ar = '$i2'");
...
$sql = mysqli_query($conecta, "SELECT foto FROM artista WHERE id = '$i2';");
...
unlink("fotos/min/".$usuario->foto."");
	...
	if (!empty($foto["name"])) {
		...
		if (count($error) == 0) {
			...
			$sql = mysqli_query($conecta, "update artista set foto = '$nome_imagem' where id = '$i2';");
...
<?php
$estilo = mysqli_query($conecta, "SELECT * FROM est_musical WHERE id = '$rartista[est_musical]'");
$restilo = mysqli_fetch_array($estilo);
?>
<span class="texto">
<hr size="1" color="#cccccc">
...
if($i1 == 2){
$sql2 = mysqli_query($conecta, "SELECT * FROM post_ar WHERE id = '$i3';");
$res2 = mysqli_fetch_array($sql2);
$vnum = $res2['us'];
if($vnum == $i2){
$csql4 = mysqli_query($conecta, "DELETE FROM post_ar WHERE id='$i3'");
if($csql4) {
...
if(isset($_POST["alterar"])) { 
	...
	if(!empty($_POST["nomeband"]) && !empty($_POST["descricao"]) && !empty($_POST["est_musical"]) && !empty($_POST["musicas"]) && !empty($_POST["cidade"]) && !empty($_POST["estado"])) { 
			...
			$insert = mysqli_query($conecta, "update artista set nome = '$nomeband', descricao = '$descricao', est_musical = '$est_musical', musicas = '$musicas', cidade = '$cidade', estado = '$estado' where id = '$i2';") or die(mysqli_error());
...
<?php
$csqlopt = mysqli_query($conecta, "select * from est_musical order by nome");
while($rsqlopt = mysqli_fetch_array($csqlopt)){
