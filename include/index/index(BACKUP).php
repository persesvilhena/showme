...
if(isset($_POST["post"])) { 
	...
	if(!empty($_POST["msg"])) { 
		...
		if(isset($_GET['arquivo'])){$arquivo = $class->antisql($_GET['arquivo']);} else {$arquivo = null;}
			...
			$insert = mysqli_query($conecta, "insert into post (id_us, msg, foto, arquivo, data) values ('$row[id]', '$msg', '$foto', '$arquivo', '$date');") or die(mysqli_error()); // Insiro os dados no BD
...
if($i1 == 2){
	...
	$csql = mysqli_query($conecta, "select * from gostar where id_us = '$id' and id_post = '$i2';");
	if(mysqli_num_rows($csql) > 0){
		$insert = mysqli_query($conecta, "update gostar set gostei = '$i3', data = '$date' where id_post = '$i2' and id_us = '$id'");
		...
		$insert = mysqli_query($conecta, "insert into gostar (id_post, id_us, gostei, data) values('$i2', '$id', '$i3', '$date')");
		if($insert){ echo "<script>window.location='index.php?index';</script>"; }else{ echo "<script>alert('error');window.location='index.php?index';</script>"; }
	}
}
if($i1 == 3){
	//id_post = i2 //////// gostei i3
	$csql = mysqli_query($conecta, "select * from gostar where id_us = '$id' and id_repost = '$i2';");
	if(mysqli_num_rows($csql) > 0){
		$insert = mysqli_query($conecta, "update gostar set gostei = '$i3', data = '$date' where id_repost = '$i2' and id_us = '$id'");
		if($insert){ echo "<script>window.location='index.php?index';</script>"; }else{ echo "<script>alert('error');window.location='index.php?index';</script>"; }
	} else {
		$insert = mysqli_query($conecta, "insert into gostar (id_repost, id_us, gostei, data) values('$i2', '$id', '$i3', '$date')");
		if($insert){ echo "<script>window.location='index.php?index';</script>"; }else{ echo "<script>alert('error');window.location='index.php?index';</script>"; }
	}
}
if($i1 == 4){
	$sql2 = mysqli_query($conecta, "SELECT * FROM post WHERE id = '$i2' and id_us = '$id';");
	$res2 = mysqli_fetch_array($sql2);
	$vnum = $res2['id_us'];
	if($vnum == $row['id']){
		$csql4 = mysqli_query($conecta, "DELETE FROM post WHERE id='$i2' and id_us = '$id'");
...
if($i1 == 5){
	$sql2 = mysqli_query($conecta, "SELECT * FROM repost WHERE id = '$i2' and id_us = '$id';");
	$res2 = mysqli_fetch_array($sql2);
	$vnum = $res2['id_us'];
	if($vnum == $row['id']){
		$csql4 = mysqli_query($conecta, "DELETE FROM repost WHERE id='$i2' and id_us = '$id'");
		if($csql4) {
			$msg_erro = "Postagem apagada com sucesso!";
		}
		else {
...
if($i1 == 6){
	...
	if($vnum == $row['id']){
		if(isset($_POST["alterapost"])) {
			if(!empty($_POST["msg"])) {
				$msg = $class->antisql($_POST["msg"]);
				$csql4 = mysqli_query($conecta, "update post set msg = '$msg' WHERE id='$i2' and id_us = '$id'");
				if($csql4) {
...
if($i1 == 7){
	...
	if($vnum == $row['id']){
		if(isset($_POST["alterarepost"])) {
			if(!empty($_POST["msg"])) {
				$msg = $class->antisql($_POST["msg"]);
				$csql4 = mysqli_query($conecta, "update repost set msg = '$msg' WHERE id='$i2' and id_us = '$id'");
				if($csql4) {
					echo "<script>alert('Postagem alterada com sucesso!');window.location='index.php?index';</script>";
				}
				else {
...
$res1 = mysqli_query($conecta, "SELECT * FROM `post` where id_us in ($contatos) ORDER BY id DESC LIMIT 0 , 30");
while($escrever1=mysqli_fetch_array($res1)){
	...
	$res3 = mysqli_query($conecta, "SELECT * FROM `repost` where id_post = '$escrever1[id]' ORDER BY id asc LIMIT 0 , 30");
 	while($escrever3=mysqli_fetch_array($res3)){
 		$csql = mysqli_query($conecta, "select * from user where id = '$escrever3[id_us]';");
 		$rsql = mysqli_fetch_array($csql);
 		$datatempo2 = explode(" ", $escrever3['data']);
		$dat2 = explode("-", $datatempo2[0]);
		$like2 = mysqli_query($conecta, "select * from gostar where id_repost = '$escrever3[id]' and gostei = '1'");
		$rlike2 = mysqli_num_rows($like2);
		$nlike2 = mysqli_query($conecta, "select * from gostar where id_repost = '$escrever3[id]' and gostei = '0'");
