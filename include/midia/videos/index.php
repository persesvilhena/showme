...
if($i1 == null){$i1 = 1;}
if ($i1 == 1){
	$csql = mysqli_query($conecta, "SELECT * FROM videos WHERE id_us = '$id';");
...
if($i1 == 2){
	$csql = mysqli_query($conecta, "select * from videos where id = '$i2' and id_us = '$id';");
	...
	if(mysqli_num_rows($csql) == 1){
		if(isset($_POST["alterar"])) {
			if(!empty($_POST["nome"]) && !empty($_POST["descricao"])) {
				...
				$altera = mysqli_query($conecta, "update videos set nome = '$nome', descricao = '$descricao' where id = '$i2' and id_us = '$id';");
				if ($altera){ 
...
if($i1 == 3){
	...
	$rsql = mysqli_fetch_array($csql);
	if(mysqli_num_rows($csql) == 1){
		$altera = mysqli_query($conecta, "delete from videos where id = '$i2' and id_us = '$id';");
...
$insert = mysqli_query($conecta, "insert into videos (id_us, link, nome, descricao) values ('$id', '$nome_final', '$nome', '$descricao');");
if($insert){
