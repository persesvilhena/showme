...
if($rsecveradm['adm'] == 1){
	$csql = mysqli_query($conecta, "SELECT * FROM musicas WHERE id_ar = '$i2';");
	...
	if($rsecveradm['adm'] == 1){
		if(isset($_POST["alterar"])) {
			if(!empty($_POST["nome"]) && !empty($_POST["descricao"])) {
				...
				$altera = mysqli_query($conecta, "update musicas set nome = '$nome', descricao = '$descricao' where id = '$i2' and id_ar = '$rsql[id_ar]';");
				if ($altera){ 
...
if($i1 == 3){
	...
	$rsecveradm = mysqli_fetch_array($secveradm);
	if($rsecveradm['adm'] == 1){
		$altera = mysqli_query($conecta, "delete from musicas where id = '$i2' and id_ar = '$rsql[id_ar]';");
...
$insert = mysqli_query($conecta, "insert into musicas (id_ar, link, nome, descricao) values ('$i2', '$nome_final', '$nome', '$descricao');");
if($insert){
