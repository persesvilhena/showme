...
if($i1 == null){$i1 = 1;}
if ($i1 == 1){
	$csql = mysqli_query($conecta, "SELECT * FROM musicas WHERE id_us = '$id';");
	while($rsql = mysqli_fetch_array($csql)){
		...
		<span class=\"titulo\"><center><a href=index.php?musicas&i1=5&i2=" . $rsql['id'] . " class=classe1>" . $rsql['nome'] . "</a><br></span>
		<span class=\"texto\">
		<a href=\"#pfdialog\" name=\"pfmodal\" class=\"classe1\" onclick=\"CarregaDadosJanela('" . $rsql['id'] . "','mus_edit');\">Alterar</a> - 
		<a href=\"#pfdialog\" name=\"pfmodal\" class=\"classe1\" onclick=\"CarregaDadosJanela('" . $rsql['id'] . "','mus_del');\">Excluir</a>
		</span>
		</center>
		</div>
		";
	...
	echo "<br><div align=right><a href=\"#pfdialog\" name=\"pfmodal\" class=\"botao\" onclick=\"CarregaDadosJanela('0','mus_novo');\">Nova Música</a></div>";
...
if($i1 == 2){
	$csql = mysqli_query($conecta, "select * from musicas where id = '$i2' and id_us = '$id';");
	...
	if(mysqli_num_rows($csql) == 1){
		if(isset($_POST["alterar"])) {
			if(!empty($_POST["nome"]) && !empty($_POST["descricao"])) {
				...
				$altera = mysqli_query($conecta, "update musicas set nome = '$nome', descricao = '$descricao' where id = '$i2' and id_us = '$id';");
				if ($altera){ 
					echo "<script>alert('Musica alterada com sucesso!');window.location='index.php?musicas';</script>"; 
				}else{
					echo "<script>alert('Houve um problema!');window.location='index.php?musicas';</script>";
				}
			}else{
				echo "<script>alert('Houve um problema!');window.location='index.php?musicas';</script>";
			}
		...
		<form action=\"index.php?musicas&i1=2&i2=" . $rsql['id'] . "\" method=\"post\">
...
if($i1 == 3){
	...
	$rsql = mysqli_fetch_array($csql);
	if(mysqli_num_rows($csql) == 1){
		$altera = mysqli_query($conecta, "delete from musicas where id = '$i2' and id_us = '$id';");
		unlink("uploads/musicas/".$rsql['link']."");
		if ($altera){ 
			echo "<script>alert('Musicas apagada com sucesso!');window.location='index.php?musicas';</script>"; 
		}else{
			echo "<script>alert('Houve um problema!');window.location='index.php?musicas';</script>";
		}
	}
...
$insert = mysqli_query($conecta, "insert into musicas (id_us, link, nome, descricao) values ('$id', '$nome_final', '$nome', '$descricao');");
if($insert){
// Upload efetuado com sucesso, exibe uma mensagem e um link para o arquivo
echo "<script>alert('Upload efetuado com sucesso!');window.location='index.php?musicas'</script>";
...
echo "Não foi possível enviar o arquivo, tente novamente";
	...
	echo "<span class=\"titulo\">Nova Musica</span><hr size=\"1\" color=\"#cccccc\">
	<form action=\"index.php?musicas&i1=4\" method=\"post\">
