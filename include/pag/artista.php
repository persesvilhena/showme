...
if($i1 == null){$i1 = 1;}
if ($i1 == 1){
$csql = mysqli_query($conecta, "SELECT * FROM artista WHERE id='$i2'");
$rsql = mysqli_fetch_array($csql);
$csql2 = mysqli_query($conecta, "SELECT * FROM est_musical WHERE id='$rsql[est_musical]'");
...
";
$cont = 0;
$memsql = mysqli_query($conecta, "SELECT * FROM membros where id_ar = '$i2' ORDER BY id DESC LIMIT 0 , 100");
 while($rmemsql=mysqli_fetch_array($memsql)){
	$usersql = mysqli_query($conecta, "SELECT * FROM user WHERE id = '$rmemsql[id_us]'");
	$rusersql = mysqli_fetch_array($usersql);
	$cont = $cont + 1;
	$mod = $cont % 2;
...
$ct = 0;
$imsql = mysqli_query($conecta, "SELECT * FROM membro_instrumento where id_us = '$rmemsql[id_us]' and id_ar = '$i2' ORDER BY id DESC LIMIT 0 , 100");
...
$agendasql = mysqli_query($conecta, "SELECT * FROM agenda where id_ar = '$i2' ORDER BY id DESC LIMIT 0 , 3");
...
$agendasql = mysqli_query($conecta, "SELECT * FROM noticias where id_ar = '$i2' ORDER BY id DESC LIMIT 0 , 3");
 while($ragendasql=mysqli_fetch_array($agendasql)){
 	$dat = explode("-", $ragendasql['data']);
...
<span class=\"titulo\">Musicas:</span><br>";
	$musicascsql = mysqli_query($conecta, "SELECT * FROM musicas WHERE id_ar = '$i2' limit 0 , 3;");
...
";
$res = mysqli_query($conecta, "SELECT * FROM `album_ar` where us='$i2' limit 0,4;");
...
<span class=\"titulo\">Videos:</span><br>";
	$videoscsql = mysqli_query($conecta, "SELECT * FROM videos WHERE id_ar = '$i2' limit 0 , 2;");
...
$disccsql = mysqli_query($conecta, "SELECT * FROM disc WHERE id_ar = '$i2' limit 0,4;");
while($discrsql = mysqli_fetch_array($disccsql)){
...
$res = mysqli_query($conecta, "SELECT * FROM `album_ar` where us='$i2';");
 while($escrever=mysqli_fetch_array($res)){
...
if($rsecveradm['adm'] == 1){
$add = mysqli_query($conecta, "update user set idart = '$i2' where id = '$id';") or die(mysqli_error()); 
if($add) {
...
$rsecveradm = mysqli_fetch_array($secveradm);
if($rsecveradm['adm'] == 1){
$add = mysqli_query($conecta, "update user set idart = null where id = '$id';") or die(mysqli_error()); 
if($add) {
	echo "<script>alert('Ok!');window.location = 'index.php?verart&i1=1&i2=" . $i2 . "';</script>";
}
else {
...
if ($i1 == 7){
	...
	$videoscsql = mysqli_query($conecta, "SELECT * FROM videos WHERE id_ar = '$i2';");
	while($videosrsql = mysqli_fetch_array($videoscsql)){
		echo "
		<div id=\"item\" style=\"position: relative; width: 329px; float: left; margin: 2px; text-align: justify; padding: 5px;\">
		<div id=\"myElement" . $videosrsql['id'] . "\">Loading the player...</div>
...
if ($i1 == 8){
	...
	<hr size=\"1\" color=\"#cccccc\">";
	$musicascsql = mysqli_query($conecta, "SELECT * FROM musicas WHERE id_ar = '$i2';");
	while($musicasrsql = mysqli_fetch_array($musicascsql)){
		echo "
...
$disccsql = mysqli_query($conecta, "SELECT * FROM disc WHERE id_ar = '$i2';");
while($discrsql = mysqli_fetch_array($disccsql)){
	$cont++;
	$rest = $cont % 4;
...
while($mm_rsql = mysqli_fetch_array($mm_csql)){
	echo "
	<tr>
...
if ($i1 == 14){
$qfoto = mysqli_query($conecta, "select * from album_ar where id = '$i2';");
...
if ($i1 == 18){
	$csql = mysqli_query($conecta, "select * from musicas where id = $i2;");
	$rsql = mysqli_fetch_array($csql);
	echo "
	<span class=\"titulo\">" . $rsql['nome'] . "</span><hr size=\"1\" color=\"#cccccc\">
		<center>
...
if ($i1 == 19){
	$csql = mysqli_query($conecta, "select * from disc where id = $i2;");
	$rsql = mysqli_fetch_array($csql);
	echo "
	<span class=\"titulo\">" . $rsql['nome'] . "</span><hr size=\"1\" color=\"#cccccc\">
	<center>
	...
	";
	$cmus = mysqli_query($conecta, "select * from disc_musicas where id_album = '$i2';");
	while($rmus = mysqli_fetch_array($cmus)){
...
if ($i1 == 20){
	$csql = mysqli_query($conecta, "select * from disc_musicas where id = $i2;");
	$rsql = mysqli_fetch_array($csql);
	$qtddevotos = mysqli_query($conecta, "SELECT count(*) FROM `pt_musica` WHERE id_mus = '$i2'");
	$rqtddevotos = mysqli_fetch_array($qtddevotos);
	$medianota = mysqli_query($conecta, "SELECT avg(nota) FROM `pt_musica` WHERE id_mus = '$i2'");
	$rmedianota = mysqli_fetch_array($medianota);
