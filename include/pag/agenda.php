...
<div class=\"tabela1\">
";
$res = mysqli_query($conecta, "SELECT * FROM `agenda` ORDER BY `agenda`.`id` DESC");
echo "<table>
<tr>
...
<td width=120>Local</td>
  ...
  while($escrever=mysqli_fetch_array($res)){
  	$csql_ar = mysqli_query($conecta, "select * from artista where id = '$escrever[id_ar]';");
  	$rsql_ar = mysqli_fetch_array($csql_ar);
  	$csql_estilo = mysqli_query($conecta, "select * from est_musical where id = '$rsql_ar[est_musical]';");
  	$rsql_estilo = mysqli_fetch_array($csql_estilo);
  echo "<tr>
...
if ($i1 == 2) {
$sql2 = mysqli_query($conecta, "SELECT * FROM agenda WHERE id='$i2'");
$res2 = mysqli_fetch_array($sql2);
	if ($res2['rev'] == 0){ $rev = "Nao Revisada"; }
	if ($res2['rev'] == 1){ $rev = "Revisada"; }
$csql_ar = mysqli_query($conecta, "SELECT * FROM artista WHERE id='$res2[id_ar]'");
$rsql_ar = mysqli_fetch_array($csql_ar);
$csql_estilo = mysqli_query($conecta, "select * from est_musical where id = '$rsql_ar[est_musical]';");
$rsql_estilo = mysqli_fetch_array($csql_estilo);
echo "
