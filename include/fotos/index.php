...
$sec = mysqli_query($conecta, "select * from album where id = '$idimg'");
$rsec = mysqli_fetch_array($sec);
if ($rsec['us'] == $id){
$alte = mysqli_query($conecta, "update album set descricao = '$descricao' where id = '$idimg'");
...
if (isset($_POST['cadastrar'])) {
	...
	if (!empty($foto["name"])) {
		...
		if (count($error) == 0) {
			...
			$sql = mysqli_query($conecta, "INSERT INTO album VALUES (null, '$descricao', '$id_gen', '$nome_imagem')");
...
";
$res = mysqli_query($conecta, "SELECT * FROM `album` where us = '$id_gen' order by id desc;");
