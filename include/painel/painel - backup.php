<?php
if($z1 == 1){
	$secveradm = mysqli_query($conecta, "select * from membros where id_us = '$id' and id_ar = '$z2'");
	$rsecveradm = mysqli_fetch_array($secveradm);
	if($rsecveradm['adm'] == 1){
		$upart = mysqli_query($conecta, "update user set idart = '$z2' where id = '$id'");
		if($upart){
...
if($z1 == 2){
	$upart = mysqli_query($conecta, "update user set idart = null where id = '$id'");
	if($upart){
		echo "<script>window.location='" . $eurl . "';</script>";
	}else{
		echo "<script>alert('Error!');window.location='" . $u . "';</script>";
...
<div id="conteudo3" style="left: 0px; margin-top: 4px; width: 200px; text-align: justify; padding-bottom: 5px;" align="center">
	<div style="position: relative; left: 2px; top: 2px; text-align: justify;" align="left">
		...
		while($escrever=mysqli_fetch_array($res)){
			if($escrever['adm'] == 1){
				$cbanda = mysqli_query($conecta, "SELECT * FROM artista WHERE id = '$escrever[id_ar]'");
				$rbanda = mysqli_fetch_array($cbanda);
